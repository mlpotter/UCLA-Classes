...
parser.add_argument('--dropout',type=float,help='value to use for dropout on adjacency matrix, otherwise False',default = 0)
...
save_path = ["model_best_"+corpus+".pt","model_best_dropout_"+corpus+".pt"][dropout!=0]
...
if dropout != 0:
  model = GCN_dropout(data,hidden,dropout).to(device)
else:
...
correct = int(y_pred[data.val_mask].eq(data.y[data.val_mask]).sum().item())
val_acc = correct/int(data.val_mask.sum())
...
print("-"*25,"Test Evaluation","-"*25)
evaluation_report(y_true,y_pred,labels)
print("-"*25,"Best Validation Accuracy","-"*25)
print('Validation Accuracy: {:.6f}'.format(val_acc))
